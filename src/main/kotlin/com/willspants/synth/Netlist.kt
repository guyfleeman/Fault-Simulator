...
class Netlist(val name: String = "CKT") {
    ...
    fun propagateInputVector() {
        ...
        if (liv.size != globalInputPins.size) {
            throw java.lang.RuntimeException("input vector is not mappable to input pins. Vector Length: ${liv.size}, Num Pins: ${globalInputPins.size}")
        }
...
//        println("")
    ...
    fun getInputVectorSize(): Int {
        return globalInputPins.size
    }
}